import { readAllUsers, suspendProfile, unSuspendProfile} from '../actions/admin';
	  	...
	  	<div className="side-bar">
	  		<div className="side-bar-icons">
	  			 <div className="side-bar-logo text-center py-3">
	  				<img src={profile.image || image} className="img-fluid rounded-circle border bg-secoundry mb-3 " alt="Profile"/>
	  			</div> 
	  			<div className="icons d-flex flex-column align-items-center">
	  				<a href="/admin/hotels" className="set-width text-center display-inline-block my-1"><i className="fa fa-home"></i></a>
	  			</div>
	  		</div>
	  	</div>
              ...
              <thead>
                  ...
                  {users.map(user => (
                      ...
                      <tr key={user._id}>
                            ...
                            <td>{user.content}</td>
                            <td>{user.location}</td>
                            <td><img src={user.image} alt="hotel" width="100" height="100" className="img-fluid"/></td>
                            <td>{user.isSuspended ? "Suspended" : "UnSuspended"}</td>
                            <td> 
                            <DeleteOutlined onClick={() => handleProfileDelete(user._id)} className="text-danger" alt="edit"/>
                            {" "}
                            <StopOutlined onClick={() => handleUserSuspension(user._id)} className="text-danger" />
                            {" "}
                            <CheckOutlined onClick={() => handleUserUnSuspension(user._id)} className="text-success" />
                            </td>
                        </tr>
                    ))}
                  </tbody>
