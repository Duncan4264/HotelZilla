...
export const create = async (token, data) => {
  try {
    ...
    let comment = await axios.post(
      `${process.env.REACT_APP_API}/create-comment`,
      data,
      {
        headers: {
          // with jwt token
...
export const readComments = async (token, reviewId) => {
  try {
    let comments = await axios.get(
      `${process.env.REACT_APP_API}/comments/${reviewId}`,
      {
        headers: {
          Authorization: `Bearer ${token}`
        }
...
export const countComments = async (userId, token) => {
  try {
    let comments = await axios.get(
      `${process.env.REACT_APP_API}/user/comments/${userId}`,
      {
        headers: {
          Authorization: `Bearer ${token}`
        }
